...
- **Systematic triage (lsof on the live serve + SDK child, transcript inspection):** the session's actual cwd was `…/even_realities/colive-terminal` — *correct plumbing, wrong source*: it's where `colive serve` was started (per the runbook), not where the user/desk is. `runDesk` never wired the designed-for `AppConfig.cwd`, so `sendPrompt` carried no cwd → Core fell back to serve's `projectDir`; the banner displayed the desk's own cwd (a lie). CLAUDE.md lives one level up → Glob correctly empty. The hyphenated path was the **model** reverse-engineering a "project root" from the lossy encoded store-dir name (`_` and `/` both → `-`; `random_claude_stuff/even_realities` and `random-claude-stuff/even-realities` collide to the same store dir), "confirmed" by a **phantom empty dir** at exactly that path (created 2026-06-01 by an earlier session's same wrong guess). Not a broker/Hub/SDK bug.
- **Fixes:** `runDesk` passes `cwd: process.cwd()` (desk launch dir = new-session dir, native semantics; banner now truthful) — TDD red→green + app-level contract pin, **532 tests**, tsc 0. Phantom dir `rmdir`'d (real `random-claude-stuff/claude_stuff` sibling untouched). Runbook setup corrected (desk + native from the SAME dir, repo root) + **R3-4** scenario. Catalog **D-037**; knowledge: cwd-source + lossy-encoding-trap facts in `terminal-mode/overview.md`.
- **Surfaced, NOT changed (owner decision):** `settingSources: []` (M0 anti-hook-leak/latency) means the desk never auto-loads CLAUDE.md into context like native — and it isn't CLI-togglable. Candidate future rung.
