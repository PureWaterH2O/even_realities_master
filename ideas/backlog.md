...
- **Prompt queueing while busy (M3.x, desk-side)** — surfaced during M3.2B UAT (2026-06-03). A prompt submitted *while the assistant is still responding* currently lands in the **middle of the response** instead of being **queued and sent after the turn completes** like native Claude Code (which holds it at the bottom and submits on turn end). Desk-side native-parity UX. Likely shape: when `status` is busy/responding/thinking, the composer's submit should enqueue the line (show it pinned at the bottom as "queued") and flush on the next idle transition, rather than POSTing immediately. Wants its own design pass (single queued msg vs a real FIFO? visible "queued" affordance? interaction with `/clear` + interrupt? can the desk gate purely on its existing `status` events, or does turn-complete need exposing?). Probably pairs with the cockpit-liveness rung — both are status-line / turn-state aware.
