...
- **ink 7 input internals (paste & mouse):** 🧪 [`terminal-mode/ink7-input-internals.md`](terminal-mode/ink7-input-internals.md) — `usePaste` auto-enables bracketed paste (full string, separate channel from `useInput`); ink does NOT enable mouse but assembles SGR mouse sequences and re-emits them raw on `internal_eventEmitter`'s `'input'` channel (wheel = btn 64/65); verified ink 7.0.5 while probing M3.2A.
